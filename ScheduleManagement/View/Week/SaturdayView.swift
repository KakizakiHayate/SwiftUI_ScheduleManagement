...
struct SaturdayView: View {
    ...
    @State var changeObjectInt = 6
    @ObservedResults(SaturDayModel.self) var saturDayModel
    ...
    init(saturDayModel: SaturDayModel, migration: Migration) {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            if allObj.isEmpty {
                ...
                if let schedule = saturDayModel.first {
                    for list in schedule.satursDayList {
                        subjectArray.append(list)
                    }
                }
                
                //保存されたdateをself.dateに代入
                let objFirst = localRealm.objects(SaturDayModel.self).first!
...
struct SaturdayView_Previews: PreviewProvider {
    static var previews: some View {
        SaturdayView(saturDayModel: SaturDayModel(), migration: Migration()).environmentObject(Migration())
    }
}
