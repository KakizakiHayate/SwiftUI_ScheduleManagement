...
struct WednesdayView: View {
    ...
    @State var changeObjectInt = 3
    @ObservedResults(WednesDayModel.self) var wednesDayModel
    ...
    init(wednesDayModel: WednesDayModel, migration: Migration) {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            if allObj.isEmpty {
                ...
                if let schedule = wednesDayModel.first {
                    for list in schedule.wednesDayList {
                        subjectArray.append(list)
                    }
                }
                
                //保存されたdateをself.dateに代入
                let objFirst = localRealm.objects(WednesDayModel.self).first!
...
struct WednesdayView_Previews: PreviewProvider {
    static var previews: some View {
        WednesdayView(wednesDayModel: WednesDayModel(), migration: Migration()).environmentObject(Migration())
    }
}
