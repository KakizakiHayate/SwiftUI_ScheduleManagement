...
struct TuesdayView: View {
    ...
    @State var changeObjectInt = 2
    @ObservedResults(TuesDayModel.self) var tuesDayModel
    @EnvironmentObject var migration: Migration
    
    init(tuesDayModel: TuesDayModel, migration: Migration) {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            if allObj.isEmpty {
                ...
                if let schedule = tuesDayModel.first {
                    for list in schedule.tuesDayList {
                        subjectArray.append(list)
                    }
                }
                
                //保存されたdateをself.dateに代入
                let objFirst = localRealm.objects(TuesDayModel.self).first!
...
struct TuesdayView_Previews: PreviewProvider {
    static var previews: some View {
        TuesdayView(tuesDayModel: TuesDayModel(), migration: Migration()).environmentObject(Migration())
    }
}
